...
export class CalendarComponent implements OnInit {
  ...
  saveEvent(): void {
    ...
    this.formationService.addFormation(this.formation).subscribe(
      (response: Formation) => {
        ...
        this.formation.users.forEach(user => {
          const notification: Notification = {
            message: `Tu as une formation ${this.formation.nom} le ${this.formation.dateDebut.toLocaleDateString()}`,
            read: false,
            link: '/dashboard/employe/calendrier/'+user.id,
            users: [user]
          };
  ...
  handleEventClick(clickInfo: EventClickArg) {
    ...
    this.formEditData = this.formBuilder.group({
      ...
      editEnd: [formattedEnd || formattedStart]
    });
  ...
  editEventSave(): void {
    ...
    this.formation.dateDebut = new Date(this.formEditData.value.editStart);
    ...
    this.formationService.updateFormation(this.idFormation, this.formation).subscribe((data: Formation) => {
     // console.log(data);
      ...
      // Create notifications for each user about the update
      this.formation.users.forEach(user => {
        const notification: Notification = {
          message: `Update : changement formation ${this.formation.nom} le ${this.formation.dateDebut.toLocaleDateString()}`,
          read: false,
          link: '/dashboard/employe/calendrier/'+user.id,
          users: [user]
        };
  ...
  deleteEventData(): void {
    ...
    this.formationService.deleteFormation(+this.editEvent.id).subscribe(data => {
      ...
      // Create notifications for each user about the deletion
      this.formation.users.forEach(user => {
        const notification: Notification = {
          message: `Suppression : La formation ${this.formation.nom} le ${this.formation.dateDebut.toLocaleDateString()}`,
          read: false,
          link: '/dashboard/employe/calendrier/'+user.id,
          users: [user]
        };
